 ...
 * the License.
 */
package com.harmay.center.coding.generator.util;
...
public enum CaseFormat {
    ...
    /** Hyphenated variable naming convention, e.g., "Caps-Hyphen". */
    CAPS_HYPHEN(CharMatcher.is('-'), "-") {
        ...
        String normalizeWord(String word) {
            ...
            return Ascii.toUpperCase(word.substring(0, 1)) + Ascii.toLowerCase(word.substring(1));
        }

        @Override
        String convert(CaseFormat format, String s) {
            if (format == CAPS_UNDERSCORE) {
                return s.replace('-', '_');
            }
            return super.convert(format, s);
        }
    ...
    /** Hyphenated variable naming convention, e.g., "UPPER-HYPHEN". */
    ...
    /** C++ variable naming convention, e.g., "Caps_Underscore". */
    CAPS_UNDERSCORE(CharMatcher.is('_'), "_") {
        @Override
        String normalizeWord(String word) {
            if (word.length() <= 1) {
                return Ascii.toUpperCase(word);
